...
export default {
  ...
  data() {
    return {
      /**
       * This is common store management for accessibility
       */
      sharedState: store,
    };
  },
  computed: {
    ...
    /**
      Add extra transformation for image based on contrast mode
    */
    extraTransformation() {
      switch (this.bannerEffect) {
        case 'whiteOnBlack': {
  ...
  methods: {
    /**
      Preset configuration for 2 basic contrast cases
    */
    setBannerEffect() {
      switch (this.bannerEffect) {
        case 'whiteOnBlack': {
...
<style lang="scss" scoped>
  .banner--container {
    ...
    // We don't need CSS for img anymore to avoid overriden by any other CSS 
    // .banner--img {
